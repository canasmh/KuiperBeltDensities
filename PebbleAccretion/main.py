...
from porosity import get_porosity
...
T = max(gas_temp(R), 30)
r_c = 50 * AU_TO_CM
sigma_g =  column_density(R, m_disk=0.05 * M_SUN)  # FOR CO ICE USE M_DISK = 0.05 * M_SUN # For constant use 0.1
c_s = sound_speed(T)
H = scale_height(T, R)
...
rho_sil = 3.2
rho_ice = 0.89
kbos = StreamingInstabilityData(rho_ice=rho_ice, rho_sil=rho_sil, unit_mass=rho_g * (H ** 3))
...
n_pebbles = 50
model='bimodal'
pebbles = Pebbles(n_pebbles=n_pebbles, a_min=1e-4, a_max=1.0, rho_ice=rho_ice, rho_sil=rho_sil, model=model, scale_height=H, gas_density=rho_g)
...
t_max = 5.76e6 * YRS_TO_SEC  # Maximum time of simulation
# For CO ice use 5.728e6 * YRS_TO_SEC
# For constant use 5.8271e6 // okay looks like you have to use 5.8278e6 now..
...
result["t"] = [t] 
result["dt"] = [dt]
result["mass"] = kbos.mass
result["ice_fraction"] = kbos.ice_fraction
result["density"] = kbos.density
result["porosity"] = kbos.porosity
...
while t < t_max and max(kbos.mass) < m_max:
    ...
    for m in range (n_mass):
        ...
        # TODO: FIX POROSITY MODEL (I.E., INTERPOLATE ACROSS ICE FRACTIONS)
        # new_porosity = get_porosity(kbos.radius(i=m) * 1e-5, ice_fraction=1 -kbos.ice_fraction[m])
        ...
        # print(kbos.radius(i=m) * 1e-5, 1 -kbos.ice_fraction[m], new_porosity)
    ...
    if it % icadence == 0:
        ...
        result["t"].append(t + dt) 
        result["dt"].append(dt)
        result["mass"] = np.append(result["mass"], kbos.mass).reshape(i + 2, n_mass)
        result["ice_fraction"] = np.append(result["ice_fraction"], kbos.ice_fraction).reshape(i + 2, n_mass)
        result["density"] = np.append(result["density"], kbos.density).reshape(i + 2, n_mass)
        result["porosity"] = np.append(result["porosity"], kbos.porosity).reshape(i + 2, n_mass)
        i += 1
